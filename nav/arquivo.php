...
<!--         <button class="btn btn-success" name="btnwhats" type="submit"><a href="https://api.whatsapp.com/send?phone=5591984620807&text=<?php echo $arquivo ?>">  Enviar Caixa por Whatsapp </a></button>-->
    <div class="card-deck mb-3 text-center">
        <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
                <?php foreach ($vendadao->entradaGeralarq() as $venda) : ?>
                ...
                <?php endforeach ?>
                <?php foreach ($vendadao->saidaarq() as $venda) : ?>
        ...
        <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
                <?php
                foreach ($vendadao->totalvendaarq() as $venda) :
    ...
    <!--           <h1 class="card-title pricing-card-title">R$ 22.354,00 <small class="text-muted">/ Lucro</small></h1>-->
             ...
             <?php
                foreach ($vendadao->totalvendaIfoodarq() as $venda) :
                    if ($venda->getTipo() == 'entrada') {
                        @$entrada = $venda->getTotal();
                    }
                    if ($venda->getTipo() == 'saida') {
 ...
 <!--INICIO CAIXA -->
        <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
                ...
                <ul class="list-unstyled mt-3 mb-4">
                    <?php
                    foreach ($vendadao->dinheiroarq() as $venda) :
                        if ($venda->getTipo() == 'entrada') {
                    ...
                    <?php
                    foreach ($vendadao->debitoarq() as $venda) :
                        if ($venda->getTipo() == 'entrada') {
                    ...
                    <?php
                    foreach ($vendadao->ticketarq() as $venda) :
                        if ($venda->getTipo() == 'entrada') {
        ...
        <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
                ...
                foreach ($vendadao->lucroarq() as $lucro) : ?> 
                    <?php
                    if ($lucro->getCat() == "3") {
...
//                             @$picpay = $entrada - $saida;
     ...
     <?php foreach ($pagamentodao->readarq() as $funci) : ?> 
                    <?php
                ...
                <h3 class="card-title pricing-card-title">R$  <?= number_format(@$jasaiu, 2, ',', '.') ?> <small class="text-muted"> Já saiu</small></h3>
                              <?php foreach ($pagamentodao->readDiariaarq() as $funci) : ?> 
                    <?php
                    if ($funci->getIdFunci() == "6") {
                        @$diaria += $funci->getTotal();
                    } endforeach
                ?>
                <h3 class="card-title pricing-card-title">R$  <?= number_format(@$diaria, 2, ',', '.') ?> <small class="text-muted"> Entregador(Semanal)</small></h3>
                          <?php
...
<!--         <form class="needs-validation"  method="POST" action="./controller/MovController.php" >      
    ...
    <table class="table">
        ...
        <tbody class="table-group-divider">
            <?php foreach ($movdao->movimentacaoarq() as $mov) : ?>          
                <tr>
                    <?php
...
<!--                             <td > 
       ...
       <!-- Modal EDITAR MOVIMENTACAO ENTRADA -->
        ...
        <?php foreach ($pagamentodao->readPicpayarq() as $mov) : ?>          
            <tr>             
                <?php      
                $datae = new DateTime($mov->getData());
                ?>
...
//         }
    ...
    <?php foreach ($vendadao->picpayarq() as $venda) : ?> 
...
<!--                        <th>Emmanoel</th>-->
                ...
                <tbody>
                     ...
                     foreach ($pagamentodao->readarq() as $funci) : ?> 
...
$arquivo .= "Diárias Semanal(Entregador): *R$ " . number_format(@$diaria,2,",",".") . "*%0A";
   ...
   $arquivo .= "*-------- JÁ RECEBEU--------* %0A";  
     ...
     if ($funci->getIdFunci() <= "4") { 
                  ...
                  if ($funci->getIdFunci() <= "4") { 
                        ...
                        $picpayadEA = $picpayad; 
                  $arquivo .=  " *R$ " .     number_format($picpayadEA, 2, ',', '.'). "* %0A%0A" ;               
                        }
                    endforeach;         
...
//  $arquivo .= "Entregador: *R$ " . number_format($picpayadEA,2,",",".") . "* %0A%0A";
 ...
 $arquivo .= "*-------- ENVIADO DIRETO DO SISTEMA--------* %0A";
